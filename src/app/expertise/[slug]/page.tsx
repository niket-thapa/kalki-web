...
export async function generateStaticParams() {
  return pagesData.expertise.map((page) => ({
    slug: page.slug,
  }));
}

export default function ExpertisePage({ params }: PageProps) {
  const page = pagesData.expertise.find((p) => p.slug === params.slug);
...
export async function generateMetadata({ params }: PageProps) {
  ...
  return {
    ...
    description: `${page.title} expertise and solutions by Kalki Automation.`,
  };
}
